  ...
  if (scene.adjacent_path && (laneWidthLeft != 0 || laneWidthRight != 0)) {
    ...
    auto paintLane = [&](const QPolygonF &lane, float laneWidth, bool blindspot) {
      ...
      if (scene.adjacent_path_metrics) {
        ...
        QString text = blindspot ? tr("盲點偵測到車輛") : QString::number(laneWidth * distanceConversion, 'f', 2) + leadDistanceUnit;
...
void AnnotatedCameraWidget::drawStatusBar(QPainter &p) {
  ...
  std::map<int, QString> conditionalStatusMap = {
    ...
    {5, tr("條件式實驗模式覆蓋")},
    {6, tr("實驗模式手動啟動")},
    {7, tr("實驗模式啟動因") + (mapOpen ? tr(" 低速") : tr(" 速度低於 ") + QString::number(conditionalSpeedLead) + leadSpeedUnit)},
    {8, tr("實驗模式啟動因") + (mapOpen ? tr(" 低速") : tr(" 速度低於 ") + QString::number(conditionalSpeed) + leadSpeedUnit)},
    {9, tr("實驗模式啟動因 轉動") + (mapOpen ? "" : tr(" / 車道變換"))},
    {10, tr("實驗模式啟動因 路口")},
    {11, tr("實驗模式啟動因 即將轉彎")},
    {12, tr("實驗模式啟動因 彎道")},
    {13, tr("實驗模式啟動因 停止車")},
    {14, tr("實驗模式啟動因 慢速車")},
    {15, tr("實驗模式啟動因 停止標誌") + (mapOpen ? tr("") : tr(" 或停止訊號"))},
    {16, tr("實驗模式啟動因 強制停止") + (mapOpen ? tr("") : tr(" 或停止訊號"))},
    {17, tr("實驗模式啟動因 無速限")},
  };
