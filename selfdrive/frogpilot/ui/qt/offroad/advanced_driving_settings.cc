    ...
    if (param == "AdvancedLateralTune") {
      ...
      QObject::connect(blacklistBtn, &FrogPilotButtonsControl::buttonClicked, [this](int id) {
        ...
        if (id == 0) {
          ...
          QString selectedModel = MultiOptionDialog::getSelection(tr("選擇要從黑名單中刪除的型號"), blacklistedModels, "", this);
          if (!selectedModel.isEmpty()) {
            if (ConfirmationDialog::confirm(tr("您確定要刪除 '%1' 黑名單中的型號?").arg(selectedModel), tr("刪除"), this)) {
              if (blacklistedModels.contains(selectedModel)) {
