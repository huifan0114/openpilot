...
FrogPilotLongitudinalPanel::FrogPilotLongitudinalPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  const std::vector<std::tuple<QString, QString, QString, QString>> longitudinalToggles {
    ...
    {"CustomCruise", tr("巡航定速增減"), tr("Interval used when increasing the cruise control speed."), ""},
    {"CustomCruiseLong", tr("自訂巡航定速增減（長壓）"), tr("Interval used when increasing the cruise control speed when holding down the button for 0.5+ seconds."), ""},
    {"MapGears", tr("將加速/減速映射到齒輪"), tr("將加速和減速曲線映射到“Eco”或“Sport”設定模式."), ""},
    {"OnroadDistanceButton", tr("駕駛風格設定按鍵"), tr("目前的駕駛個性顯示在螢幕上。點擊切換個性，或長按2.5秒啟動“交通模式”."), ""},
    ...
    {"SLCFallback", tr("備援設定"), tr("選擇沒有可用速度限制資料時發生的情況."), ""},
