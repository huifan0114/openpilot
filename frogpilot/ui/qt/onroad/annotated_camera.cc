...
////////////NAV語音////////////////////////
  ...
  speedUnit =  s.scene.is_metric ? tr("公里") : tr("mph");
...
void AnnotatedCameraWidget::drawHud(QPainter &p) {
  ...
  if (has_us_speed_limit) {
    ...
    if (speedLimitChanged && !(setSpeed - mtscSpeed > 1 || setSpeed - vtscSpeed > 1)) {
      ...
      p.setFont(InterFont(28, QFont::Normal));
      p.drawText(new_sign_rect.adjusted(0, 22, 0, 0), Qt::AlignTop | Qt::AlignHCenter, tr("待處理"));
      p.drawText(new_sign_rect.adjusted(0, 51, 0, 0), Qt::AlignTop | Qt::AlignHCenter, tr("速限"));
      p.setFont(InterFont(70, QFont::Normal));
...
///////////////////////////////////////////////
  if (hfopinf){
    ...
    p.setPen(QPen(QColor(Qt::black), 6));
    ...
    QString autoaccprofile_text = autoaccProfileMapMap[autoacc]+" ACC";
...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s, float v_ego) {
  ...
  if (scene.show_stopping_point && scene.red_light && speed > 1 && !(conditionalStatus == 1 || conditionalStatus == 3 || conditionalStatus == 5)) {
    ...
    if (scene.show_stopping_point_metrics) {
      ...
      // static double previousStopDistance = -1;  // 初始化為 -1，這樣第一次執行時不會有錯誤

      double stopDistance = std::nearbyint(modelLength * distanceConversion);

      int lastStopDistance = -1;

      if (lastStopDistance == -1 || (lastStopDistance - stopDistance >= 10)) {
        if (stopDistance < 100 && stopDistance >= 0) { // 確認 stopDistance 有有效值
            // 只有當 stopDistance 持續減少時才紀錄
            int stopmarkDistance = stopDistance;
            params_memory.putBool("StopmarkOn", true);
            params_memory.putInt("stopmarkDistance", stopmarkDistance);

            // 更新 lastStopDistance
            lastStopDistance = stopDistance;
        // } else {
        //     // 若 stopDistance 沒有持續減少，或無效值，將 StopmarkOn 設為 false
        //     params_memory.putBool("StopmarkOn", false);
        }
    // } else if (stopDistance == 0 ){
    //     // 若 stopDistance 沒有有效數值時，將 StopmarkOn 設為 false
    //     params_memory.putBool("StopmarkOn", false);
    }

      // previousStopDistance = stopDistance;  // 更新上一個 stopDistance
      painter.save();
...
void AnnotatedCameraWidget::updateFrogPilotVariables(int alert_height, const UIScene &scene) {
  if (is_metric || useSI) {
    ...
    leadDistanceUnit = tr(mapOpen ? "米" : "公尺");
