...
/////////////////停車標記//////////////////
  ...
  if (frogpilot_toggles.value("show_stopping_point_metrics").toBool() && params.getBool("Stopmarkslowsdown")) {
    ...
    p.setFont(font);
    QRect line1Rect = debugRect.adjusted(padding, padding, -padding, -padding);
    QRect line2Rect = line1Rect.translated(0, lineHeight + 6);
    p.drawText(line1Rect, Qt::AlignLeft | Qt::AlignTop, line1);
    p.drawText(line2Rect, Qt::AlignLeft | Qt::AlignTop, line2);
    p.restore();
  }
...
/////////////////////////////////////////////////////////////////////
  // 動態更新速限來源的底部位置
  speedLimitSourcesBottomY = current_y;
...
void FrogPilotAnnotatedCameraWidget::paintVehicleInfoPanel(QPainter &p, const cereal::CarState::Reader &carState, const QJsonObject &frogpilot_toggles) {
  ...
  const QFont title_font = InterFont(45, QFont::Normal);
  const QFont body_font = InterFont(40, QFont::Normal);
  ...
  const int road_y = y_cursor;
  y_cursor += title_fm.height() + title_to_body_gap;
  const int acc_y = y_cursor;
  y_cursor += body_fm.height() + body_line_gap;
  const int profile_y = y_cursor;
  y_cursor += body_fm.height() + body_line_gap;
  const int vr_y = y_cursor;
  y_cursor += body_fm.height() + body_line_gap;
  ...
  const int oil_y = y_cursor;
  y_cursor += body_fm.height() + body_line_gap;
  const int oil_temp_y = y_cursor;
  y_cursor += body_fm.height() + body_line_gap;
  const int voltage_y = y_cursor;
  y_cursor += body_fm.height() + body_line_gap;
  const int autoacc_y = y_cursor;
  ...
  if (panel_top + panel_height > panel_bottom_limit) {
    panel_top = panel_bottom_limit - panel_height;
  }
  panel_top = std::max(panel_top, 20);
  ...
  p.setFont(body_font);
  p.drawText(info_rect.adjusted(left_padding, vr_y, 0, 0), Qt::AlignTop | Qt::AlignLeft, vr_text);
  ...
  p.setFont(body_font);
  p.drawText(info_rect.adjusted(left_padding, oil_y, 0, 0), Qt::AlignTop | Qt::AlignLeft, "油量  " + tankVolStr);
  ...
  p.drawText(info_rect.adjusted(left_padding, voltage_y, 0, 0), Qt::AlignTop | Qt::AlignLeft, "電壓  " + batteryVolStr);
...
void FrogPilotAnnotatedCameraWidget::paintLearningPanel(QPainter &p, int panel_bottom, int panel_left) {
  ...
  bool style_learned = style_stats.value("learned").toBool(false);
  int style_samples = style_stats.value("sample_count").toInt(0);
  double avg_accel = style_stats.value("avg_accel").toDouble(0.0);
  double avg_gas = style_stats.value("avg_gas").toDouble(0.0);
  double p90_accel = style_stats.value("p90_accel").toDouble(0.0);

  QString factor_text = has_style_stats ? QString::number(style_factor, 'f', 2) : "–";
  QString delta_text = has_style_stats ? QString("%1%2").arg(style_delta >= 0 ? "+" : "").arg(QString::number(style_delta, 'f', 2)) : "–";
  QString style_line1 = QString("學習  %1 (%2)").arg(factor_text, delta_text);
  QString style_line2 = QString("樣本  %1  %2").arg(style_samples).arg(style_learned ? "已學" : "學習中");
  QString accel_text = has_style_stats ? QString::number(avg_accel, 'f', 2) : "–";
  QString gas_text = has_style_stats ? QString::number(avg_gas, 'f', 2) : "–";
